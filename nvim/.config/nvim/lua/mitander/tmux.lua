local function run_tmux_script(script_name, args)
	local script = vim.fn.expand("~/dotfiles/scripts/" .. script_name)
	...
	if vim.fn.executable("tmux") ~= 1 or vim.fn.executable(script) ~= 1 then
		vim.notify("tmux script not executable: " .. script, vim.log.levels.ERROR, { title = "tmux" })
		return
	end
...
local function open_project_role(role)
	return function()
		run_tmux_script("tmux-role-window.sh", { role, vim.fn.getcwd() })
	end
end
...
return {
	...
	keys = {
		...
		{ "<leader>Ta", open_project_role("pi"), desc = "Tmux project pi window" },
		{
			"<leader>TA",
			function()
				run_tmux_script("tmux-pi-split.sh", { vim.fn.getcwd() })
			end,
